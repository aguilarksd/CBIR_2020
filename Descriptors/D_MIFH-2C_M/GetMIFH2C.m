function [MIFH] = GetMIFH2C(img,bh,bs,bv,Nv,subS,dx,dy)
...
%// Modificación (Dividir la imagen)
[r,c]=size(img(:,:,1));
r=ceil(r/3);c=ceil(c/3);% 3*3=9 cuadrants
r3=r*3;c3=c*3;
img=imresize(img,[r3 c3]);
Chsv=rgb2hsv(img);
Hw=Get_W(img,r,c,9); % Function for weights 9 cuadrants
%// Color Feature Extraction and the Initial Feature Integration
M0=GetQuantHSV(Chsv,r3,c3,bh,bs,bv);
...
G0q=Edge_Quant(sqrt(G0.^2),2);
G1q=Edge_Quant(sqrt(G1.^2),2);
G2q=Edge_Quant(sqrt(G2.^2),2);
G3q=Edge_Quant(sqrt(G3.^2),2);
...
Hc1=Co_hist(M0(1:r,1:c),Nc,dx,dy)*Hw(1);
Hc2=Co_hist(M0(1:r,c+1:c*2),Nc,dx,dy)*Hw(2);
Hc3=Co_hist(M0(1:r,c*2+1:c*3),Nc,dx,dy)*Hw(3);
Hc4=Co_hist(M0(r+1:r*2+1,1:c),Nc,dx,dy)*Hw(4);
Hc5=Co_hist(M0(r+1:r*2+1,c+1:c*2),Nc,dx,dy)*Hw(5);
Hc6=Co_hist(M0(r+1:r*2+1,c*2+1:c*3),Nc,dx,dy)*Hw(6);
Hc7=Co_hist(M0(r*2+1:r*3,1:c),Nc,dx,dy)*Hw(7);
Hc8=Co_hist(M0(r*2+1:r*3,c+1:c*2),Nc,dx,dy)*Hw(8);
Hc9=Co_hist(M0(r*2+1:r*3,c*2+1:c*3),Nc,dx,dy)*Hw(9);

H0=Hc1+Hc2+Hc3+Hc4+Hc5+Hc6+Hc7+Hc8+Hc9;

%// Feature Representation
H1=Co_hist(M1,Nv,dx,dy);
